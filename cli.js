...
async function init(packageJson, queries, options) {
  ...
  const downloadQueue = new AsyncQueue(
    ...
    async ({ track, meta, feedMeta, trackLogger }) => {
      ...
      const imageFile = !options.cover
        ? null
        : await fileMgr({
            filename: `freyrcli-${meta.fingerprint}.x4i`,
            tempdir:
              Config.dirs.cacheDir === "<tmp>"
                ? undefined
                : Config.dirs.cacheDir,
            dirname: baseCacheDir,
            keep: true,
          });
      const imageBytesWritten = !options.cover
        ? null
  ...
  const embedQueue = new AsyncQueue(
    ...
    async ({ track, meta, files, audioSource }) => {
      return Promise.promisify(atomicParsley)(meta.outFilePath, {
        ...
        artwork: options.cover ? files.image.file.name : undefined, // covr
        // sortOrder: [
        //   ['name', 'NAME'], // sonm
        //   ['album', 'NAME'], // soal
        //   ['artist', 'NAME'], // soar
        ...
        .finally(() => options.cover && files.image.file.removeCallback())
...
async function main(argv) {
  let packageJson = JSON.parse(
    fs.readFileSync(xpath.join(__dirname, "package.json")).toString()
  );
